...
Info() {
    # Informational messages only on `set -x`
    > /dev/null
}
...
aptitude --disable-columns --display-format "%p" search '?and(?installed, ?not(?automatic))' \
...
apt-get purge -y $(aptitude --disable-columns --display-format "%p" search '?config-files')
# Clean package cache
apt-get clean
...
# shellcheck disable=SC2046
apt-get purge -qq -y $(aptitude --disable-columns --display-format "%p" search '?and(?installed, ?or(?name(^task-), ?name(^tasksel)))')
...
set +x
for DEP in $(aptitude --disable-columns --display-format "%p" search \
...
STANDARD_PACKAGES="$(aptitude --disable-columns --display-format "%p" search \
...
MISSING_RECOMMENDS="$(aptitude --disable-columns --display-format "%p" search '?and(?reverse-recommends(?installed), ?version(TARGET), ?not(?installed))' \
...
MANUALLY_INSTALLED="$(aptitude --disable-columns --display-format "%p" search \
...
# shellcheck disable=SC2046
apt-get purge -qq -y $(aptitude --disable-columns --display-format "%p" search '?installed' | grep -Ex "$STANDARD_BLACKLIST" || true)
# Exim bug
...
{
    aptitude --disable-columns --display-format "%p" search '?and(?essential, ?not(?installed))'
    aptitude --disable-columns --display-format "%p" search '?and(?priority(required), ?not(?installed))'
    aptitude --disable-columns --display-format "%p" search '?and(?priority(important), ?not(?installed))'
    aptitude --disable-columns --display-format "%p" search '?and(?priority(standard), ?not(?installed))' | grep -Evx "$STANDARD_BLACKLIST"
} 2>&1 | tee missing.pkgs | grep "." && echo "Missing packages" 1>&2

Info "Check for extra packages"

{
    aptitude --disable-columns --display-format "%p" search '?garbage' | sed 's/$/ # garbage/'
    aptitude --disable-columns --display-format "%p" search '?broken' | sed 's/$/ # broken/'
    aptitude --disable-columns --display-format "%p" search '?obsolete' | sed 's/$/ # obsolete/'
    aptitude --disable-columns --display-format "%p" search "$OLD_PACKAGE_QUERY" | sed 's/$/ # old/'
    aptitude --disable-columns --display-format "%p" search '?and(?installed, ?not(?origin(Debian)))' | sed 's/$/ # non-Debian/'
    #Ubuntu: aptitude --disable-columns --display-format "%p" search '?and(?installed, ?not(?origin(Ubuntu)))' | sed 's/$/ # non-Ubuntu/'
    dpkg -l | grep "\~[a-z]\+" | grep -Ev "^ii  (${TILDE_VERSION})\s" | cut -c 1-55 | sed 's/$/ # tilde version/'
    # "-dev" versioned packages
    aptitude --disable-columns --display-format "%p" search '?and(?installed, ?name(-dev))' | sed 's/$/ # development/'
