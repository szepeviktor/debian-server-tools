...
grep -qFxr "${NEW_DOMAIN}" /etc/courier/hosteddomains || echo "[WARNING] This domain is not hosted here (${NEW_DOMAIN})" 1>&2

# Account home folder and maildir
install -o "$VIRTUAL_UID" -g "$VIRTUAL_UID" -m "u=rwx" -d "${MAILROOT}/${NEW_DOMAIN}/${EMAIL%%@*}" || Error 12 "Failed to create home: (${MAILROOT}/${NEW_DOMAIN})"
sudo -u virtual -- maildirmake "$NEW_MAILDIR" && echo "Maildir OK." || Error 15 "Cannot create maildir (${NEW_MAILDIR})"

# Quota
sudo -u virtual -- maildirmake -q 1073741824S "$NEW_MAILDIR" && echo "Quota 1 GB" || Error 16 "Cannot set quota"
