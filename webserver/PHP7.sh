...
apt-get install -y build-essential autoconf pkg-config re2c bison \
    ...
    libmysqlclient-dev librecode-dev
# Hack.
...
./configure --prefix=/opt/phpng --with-config-file-path=/opt/phpng/etc \
    --enable-fpm --enable-mbstring --enable-zip --enable-bcmath --enable-pcntl \
    --enable-ftp --enable-exif --enable-calendar --enable-sysvmsg --enable-sysvsem \
    --enable-sysvshm --enable-wddx --enable-gd-native-ttf --enable-gd-jis-conv \
    --with-curl --with-mcrypt --with-iconv --with-gmp --with-gd --with-jpeg-dir=/usr \
    --with-png-dir=/usr --with-zlib-dir=/usr --with-xpm-dir=/usr \
    --with-freetype-dir=/usr --with-openssl --with-gettext=/usr --with-zlib=/usr \
    --with-bz2=/usr --with-recode=/usr --with-mysqli=/usr/bin/mysql_config \
    --without-t1lib
make
#make test
make install

# https://packages.debian.org/jessie/php5-fpm
wget http://http.debian.net/debian/pool/main/p/php5/php5_5.6.7+dfsg-1.debian.tar.xz
