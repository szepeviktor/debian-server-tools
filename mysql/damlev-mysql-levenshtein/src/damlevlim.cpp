...
**					other functions (damlevlim() and damlevlim_deinit()) -
...
**					to each invocation of damlevlim(); pointer to a char
...
my_bool damlevlim_init(UDF_INIT *initid, UDF_ARGS *args, char *message)
{
   ...
   workspace = new int[(args->lengths[0] + 1) * (args->lengths[1] + 1)];
	...
	if (workspace == NULL)
	{
		strcpy(message, "Failed to allocate memory for damlevlim function");
		return 1;
	}
...
/******************************************************************************
** purpose:		deallocate memory allocated by damlevlim_init(); this func
**					is called once for each query which invokes DAMLEVLIM(),
**					it is called after all of the calls to damlevlim() are done
** receives:	pointer to UDF_INIT struct (the same which was used by
**					damlevlim_init() and damlevlim())
** returns:		nothing
******************************************************************************/
void damlevlim_deinit(UDF_INIT *initid)
{
...
******************************************************************************/
longlong damlevlim(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                     char *error)
{
